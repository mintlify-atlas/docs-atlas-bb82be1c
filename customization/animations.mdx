---
title: 'CSS Animations: Marquee, Blink, Caution, and EQ'
sidebarTitle: 'Animations'
description: 'Retro Webpage includes five custom CSS animations — marquee scrolling, blinking text, moving caution stripes, EQ bar bounce, and the sparkle cursor trail.'
---
...
The marquee animation slides an inline text element from fully off-screen right (`translateX(100%)`) to fully off-screen left (`translateX(-100%)`), producing the classic `<marquee>` tag behavior that GeoCities authors loved. The element must be `inline-block` with `white-space: nowrap` to prevent line breaks during travel.
...
@keyframes move-caution {
  0%   { background-position: 0 0; }
  100% { background-position: 40px 0; } /* must equal background-size width */
}
```
...
// ↑ retro-teal   retro-pink  retro-lime  amber-400
