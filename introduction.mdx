...
Each of the 16 components is a **standalone React functional component** exported from a matching file in the `components/` directory. For example, `Layout.js` exports the `Layout` wrapper component (`Ip`) that every page uses; it composes `SparkleCursor`, `TiledBackground`, and `WebringNav` internally and renders tab-bar navigation driven by the React Router `useLocation` hook.
