...
<Note>
  The `content` prop accepts any ReactNode — you can pass raw `<p>` tags, `<img>` elements, or even nested components. The blog uses `React.Fragment` wrappers with multiple `<p>` children.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Content Body" icon="paragraph">
    The `content` prop renders inside a `bg-white p-4 font-sans text-sm leading-relaxed` div. Pass a `React.Fragment` with multiple `<p>` tags for multi-paragraph entries, or any other valid JSX.
  </Card>
</CardGroup>
